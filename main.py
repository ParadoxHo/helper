...
# --- Разрешаем все источники для CORS (можно потом сузить) ---
ALLOWED_ORIGINS = ["*"]
...
def search_instructions(query: str, top_k: int = 10) -> List[Tuple[str, Dict]]:
    """Zwiększamy liczbę zwracanych fragmentów do 10."""
    if not is_rag_ready:
...
async def chat(request: ChatRequest):
    ...
    frags = search_instructions(request.message, top_k=10)  # zwiększone do 10
    ...
    # === NOWY SYSTEM PROMPT – DOKŁADNY, Z PRZYKŁADEM ===
    system_prompt = f"""
...
Przykład poprawnej odpowiedzi (opartej na kontekście):
Jeśli kontekst mówi: "Aby zmienić kod użytkownika w centrali Paradox, należy wejść w tryb programowania: [KOD SERWISOWY] + [0], następnie wybrać użytkownika, wpisać nowy kod i zatwierdzić [ENTER]."
To odpowiedź powinna brzmieć:
1️⃣ Wprowadź kod serwisowy (domyślnie 1234) i naciśnij [0].
2️⃣ Wybierz numer użytkownika (np. 001) i naciśnij [*].
3️⃣ Wprowadź nowy kod (4–6 cyfr) i naciśnij [*].
4️⃣ Naciśnij [ENTER] aby zapisać i wyjść.
...
3. **Opisuj każdy krok od początku, łącznie z wejściem w odpowiedni tryb, jeśli jest wymagane. Jeśli instrukcja podaje domyślny kod, użyj go.**
...
6. Odpowiedź powinna zawierać maksymalnie 5 kroków.
                ...
                try:
                    resp = await client.post(
                        ...
                        json={
                            ...
                            "messages": history,
                            "temperature": 0.1,          # niska temperatura
                            "max_tokens": 500,            # nieco więcej miejsca na szczegółowe kroki
                            "top_p": 0.9
                        },
                        timeout=30.0
                    )
