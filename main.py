...
# --- Временно разрешаем все источники (можно сузить позже) ---
ALLOWED_ORIGINS = ["*"]
...
async def chat(request: ChatRequest):
    ...
    # Обрезаем каждый фрагмент до 500 символов и ограничиваем общий контекст 3000 символов
    ...
    for i, (d, m) in enumerate(zip(docs, metas)):
        ...
        if total_chars + len(part) > MAX_CONTEXT_CHARS:
            context_parts.append("... (kontekst przycięty z powodu limitu)")
            break
        context_parts.append(part)
        total_chars += len(part)
    context = "\n\n---\n".join(context_parts) if context_parts else "Brak dodatkowego kontekstu."

    # Системный промпт – сокращённый, но с важными инструкциями
    system_prompt = f"""
...
- **Rosslare AC-B31**: Zmiana kodu istniejącego użytkownika wymaga: (1) usunięcia użytkownika (punkt 8), (2) dodania go z nowym kodem (punkt 7). Opisz obie procedury, łącznie z wejściem w tryb programowania (# przez 2 sekundy) i użyciem Kodu Programowania.
- **Paradox, Satel**: Opisz dokładnie każdy krok od początku.
...
"""
    ...
    # Управление историей – оставляем только 5 последних сообщений
    if not history:
