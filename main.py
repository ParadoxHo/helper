...
app = FastAPI(
    title="Wsparcie Techniczne AZS z RAG (finalna wersja)",
    lifespan=lifespan
)
...
async def chat(request: ChatRequest):
    ...
    # Логируем найденные фрагменты для отладки
    if docs:
        print("📚 Znalezione fragmenty (pierwsze 500 znaków):")
        for i, (d, m) in enumerate(zip(docs, metas)):
            print(f"   [{i}] Źródło: {m.get('source', 'nieznany')}, urządzenie: {m.get('device', 'nieznane')}")
            print(f"       {d[:500]}...")
    else:
        print("📭 Nie znaleziono żadnych fragmentów")

    # Ограничение длины контекста для предотвращения ошибки 413
    ...
    # --- MAKSYMALNIE ŚCISŁY SYSTEM PROMPT ---
    system_prompt = f"""
Jesteś inżynierem wsparcia technicznego. Twoim zadaniem jest pomóc operatorowi stacji benzynowej rozwiązać problem.

**Masz dostęp WYŁĄCZNIE do poniższego kontekstu (fragmentów instrukcji). Nie używaj żadnej innej wiedzy.**
Jeśli pytanie dotyczy urządzenia, a w kontekście nie ma informacji na ten temat – odpowiedz: "Nie znalazłem tej informacji w instrukcjach. Proszę skontaktować się z serwisem."

KONTEKST (tylko to może być użyte do odpowiedzi):
{context}

**WAŻNE ZASADY**:
1. Jeśli pytanie dotyczy **Paradox**, a w kontekście znajdują się fragmenty dotyczące Paradox – użyj TYLKO ich. Ignoruj fragmenty dotyczące innych urządzeń, nawet jeśli są w kontekście.
2. Jeśli pytanie dotyczy **Rosslare**, a w kontekście są fragmenty Rosslare – użyj TYLKO ich.
3. Jeśli pytanie dotyczy **Satel**, a w kontekście są fragmenty Satel – użyj TYLKO ich.
4. Podobnie dla Bosch, Siemens, 3xLogic, Provision, Hikvision, Babyware.
5. Jeśli w kontekście są fragmenty z różnych urządzeń, a pytanie nie wskazuje urządzenia – zapytaj o model.
6. Jeśli kontekst zawiera fragmenty z urządzenia innego niż to, o które pyta użytkownik – ZIGNORUJ JE. Nie mieszaj procedur.

**FORMAT ODPOWIEDZI**:
- Odpowiadaj TYLKO po polsku.
- Każdy krok pisz w osobnej linii, poprzedzony emoji: 1️⃣, 2️⃣, 3️⃣ itd.
- Między krokami zostaw pustą linię (akapit), aby odpowiedź była czytelna.
- Opisuj każdy krok od początku, łącznie z wejściem w tryb programowania i użyciem wszystkich kodów.
- Maksymalnie 10 kroków.
- Nie dodawaj wstępów, podsumowań ani wyjaśnień technicznych.

**PRZYKŁAD POPRAWNEJ ODPOWIEDZI DLA ROSSLARE** (jeśli kontekst zawiera tę instrukcję):
...
**PRZYKŁAD DLA PARADOX** (jeśli kontekst zawiera instrukcję):
[Jeśli w kontekście jest procedura uzbrajania, przepisz ją dokładnie, krok po kroku. Jeśli jej nie ma, odpowiedz zgodnie z zasadą nr 1.]

**PAMIĘTAJ**: Twoja odpowiedź musi być oparta WYŁĄCZNIE na kontekście. Jeśli kontekst nie zawiera pełnej procedury, nie próbuj jej uzupełniać.
"""

    # Jeśli kontekst pusty, a urządzenie wykryte – szybka odpowiedź
    ...
    # Zarządzanie historią (systemowy + ostatnie 5)
    if not history:
