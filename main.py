...
# --- Модель эмбеддингов (полное имя, поддерживаемое fastembed) ---
...
def get_embedding_model():
    ...
    if embedding_model is None:
        print("⏳ Ładowanie modelu fastembed (może potrwać)...")
        try:
...
def split_text_into_chunks(text: str, chunk_size: int = 150, overlap: int = 20) -> List[str]:
    """Ещё меньший размер чанков для экономии памяти"""
    words = text.split()
    chunks = []
...
async def load_instructions_async():
    """Асинхронная загрузка инструкций"""
    global is_rag_ready
    try:
        await asyncio.to_thread(load_instructions_sync)  # выполняем в отдельном потоке
    except Exception as e:
...
def load_instructions_sync():
    """Синхронная загрузка инструкций (вызывается в потоке)"""
    global collection
    init_chroma()
    ...
    for fpath in files:
        ...
        time.sleep(0.1)  # пауза после файла
...
async def reload_instructions():
    try:
        ...
        # Запускаем перезагрузку синхронно, но в фоне, чтобы не блокировать ответ
        asyncio.create_task(load_instructions_async())
