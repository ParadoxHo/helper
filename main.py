...
ALLOWED_ORIGINS = ["*"]  # Для теста, потом можно сузить
...
def search_instructions(query: str, top_k: int = 20) -> List[Tuple[str, Dict]]:
    """Zwraca do 20 najbardziej pasujących fragmentów."""
    if not is_rag_ready:
...
async def chat(request: ChatRequest):
    ...
    # Szukamy fragmentów – top_k=20
    frags = search_instructions(request.message, top_k=20)
    docs = [d for d,_ in frags]
    metas = [m for _,m in frags]

    context_parts = []
    for i, (d, m) in enumerate(zip(docs, metas)):
        context_parts.append(f"[Fragment {i+1} z pliku {m.get('source', 'nieznany')}]\n{d}")
    context = "\n\n---\n".join(context_parts) if context_parts else "Brak dodatkowego kontekstu."

    # === ULEPSZONY SYSTEM PROMPT – DOKŁADNY, ZE SPECYFIKACJĄ DLA URZĄDZEŃ ===
    system_prompt = f"""
Jesteś inżynierem wsparcia technicznego. Twoim zadaniem jest pomóc operatorowi stacji benzynowej rozwiązać problem, **korzystając WYŁĄCZNIE** z poniższego kontekstu (fragmentów instrukcji). Nie dodawaj żadnych informacji spoza kontekstu. Jeśli kontekst nie zawiera odpowiedzi, napisz: "Nie znalazłem tej informacji w instrukcjach. Proszę skontaktować się z serwisem."

KONTEKST:
{context}

**WAŻNE**: Kontekst może zawierać wiele fragmentów. Zbierz z nich wszystkie niezbędne informacje, aby utworzyć pełną, szczegółową instrukcję krok po kroku.

**INSTRUKCJE SPECJALNE DLA NIEKTÓRYCH URZĄDZEŃ**:
- **Rosslare AC-B31**: Zmiana kodu istniejącego użytkownika wymaga dwóch operacji: najpierw usunięcia użytkownika (punkt menu 8), a następnie dodania go z nowym kodem (punkt menu 7). Opisz obie procedury w odpowiedzi, łącznie z wejściem w tryb programowania (# przez 2 sekundy) i użyciem Kodu Programowania.
- **Paradox**: Zmiana kodu użytkownika odbywa się przez tryb programowania użytkownika. Opisz kroki: wejście w tryb, wybór użytkownika, wpisanie nowego kodu, zatwierdzenie.
- **Satel**: Podobnie – opisz dokładnie, jak wejść w odpowiedni tryb i zmienić kod.

Przykład poprawnej odpowiedzi (opartej na kontekście dla Rosslare):
Jeśli kontekst mówi:
Fragment 1: "Wejście do Trybu Programowania: Wciśnij i przytrzymaj przez 2 sekundy przycisk #. Dioda Mode zgaśnie, dioda Door zapali się na czerwono. Wprowadź Kod Programowania."
Fragment 2: "Usuwanie użytkowników metodą standardową: Wejdź do trybu programowania, wciśnij 8, wprowadź numer użytkownika, potwierdź Kodem Programowania."
Fragment 3: "Programowanie kodów podstawowych metodą standardową: Wciśnij 7, wprowadź numer użytkownika, wprowadź nowy kod."

To odpowiedź powinna brzmieć:
1️⃣ Wejdź w tryb programowania: naciśnij i przytrzymaj # przez 2 sekundy, dioda Mode zgaśnie, dioda Door zapali się na czerwono. Wprowadź Kod Programowania (fabrycznie 1234). Po poprawnym kodzie dioda Door zaświeci się na zielono.
...
3️⃣ Dodaj nowego użytkownika z tym samym numerem: naciśnij 7, dioda Door zaświeci się na pomarańczowo. Wprowadź ten sam numer użytkownika (001). Dioda Mode zacznie migać na zielono – wprowadź nowy czterocyfrowy kod (np. 9876). Po zaakceptowaniu dioda Mode przestanie migać.
4️⃣ Wyjdź z trybu programowania: naciśnij i przytrzymaj # przez 2 sekundy. Usłyszysz trzy sygnały, dioda Door zgaśnie, dioda Mode zaświeci się na zielono.

ZASADY:
1. Odpowiadaj TYLKO po polsku.
2. Podawaj wyłącznie krótkie, konkretne instrukcje krok po kroku, w formie numerowanej listy z emoji (1️⃣, 2️⃣, 3️⃣...).
3. **Opisuj każdy krok od początku, łącznie z wejściem w odpowiedni tryb, podaniem domyślnych kodów (jeśli znane), i dokładnym opisem, co nacisnąć.**
4. Jeśli instrukcja wymaga sprawdzenia zasilania, napisz po prostu "Sprawdź zasilanie".
5. Jeśli problemu nie da się rozwiązać (lub brak kontekstu), napisz "Skontaktuj się z serwisem".
6. Odpowiedź powinna zawierać maksymalnie 10 kroków (możesz użyć więcej, jeśli potrzeba).
7. Nie dodawaj żadnych wstępów, podsumowań ani wyjaśnień technicznych.
"""
                ...
                try:
                    resp = await client.post(
                        ...
                        json={
                            ...
                            "messages": history,
                            "temperature": 0.05,          # minimalna temperatura
                            "max_tokens": 800,            # więcej miejsca na szczegółowe odpowiedzi
                            "top_p": 0.9
                        },
                        timeout=30.0
                    )
