...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Действия при старте и завершении приложения"""
    print("🚀 Uruchamianie aplikacji...")
    # Принудительно инициализируем ChromaDB
    init_chroma()
    # Пробуем загрузить инструкции (если они есть)
    load_instructions()
    # Прогреваем модель эмбеддингов (синхронно, т.к. это необходимо)
    try:
        get_embedding_model()
    except Exception as e:
        print(f"⚠️ Nie udało się załadować modelu: {e}")
    yield
    # Здесь можно добавить действия при завершении, если нужно
    print("🛑 Aplikacja zakończona.")
# ==================================================

app = FastAPI(
    title="Wsparcie Techniczne AZS z RAG",
    lifespan=lifespan
)
...
def get_embedding_model():
    """Ленивая загрузка модели эмбеддингов (но теперь она уже установлена)"""
...
def extract_text_from_pdf(pdf_path: str) -> str:
    """Извлекает текст из PDF с обработкой ошибок кодировки"""
    text = ""
    try:
...
def split_text_into_chunks(text: str, chunk_size: int = 500, overlap: int = 50) -> List[str]:
    """Разбивает текст на перекрывающиеся кусочки"""
    words = text.split()
    chunks = []
...
def load_instructions():
    ...
    # Ищем PDF и TXT файлы (регистронезависимо)
...
async def list_instructions():
    """Показывает, какие файлы лежат в папке instructions"""
...
async def reload_instructions():
    """Принудительно перезагружает инструкции в базу"""
    try:
        # Опционально: очистить коллекцию перед загрузкой
        # if collection:
...
@app.get("/debug_env")
async def debug_env():
    """Показывает информацию о среде (для отладки)"""
    return {
