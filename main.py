...
# Модель эмбеддингов (проверенная, мультиязычная)
...
def split_text_into_chunks(text: str, chunk_size: int = 150, overlap: int = 20) -> List[str]:
    """Меньшие чанки для экономии памяти"""
    words = text.split()
    chunks = []
...
def load_instructions_sync():
    ...
    for i, chunk in enumerate(all_chunks):
        # Генерируем по одному, чтобы не перегружать память
...
# --- Testowy endpoint do sprawdzenia CORS ---
@app.get("/test-cors")
async def test_cors():
