...
def load_instructions_sync(force_reindex: bool = False):
    ...
    for fpath in files:
        ...
        # Better device tagging: filename + content
...
def search_instructions(query: str, top_k: int = TOP_K) -> List[Tuple[str, Dict]]:
    """
    Instructions-first search.
    - Try device-filtered if device found.
    - Fallback to global top results if filtered empty.
    """
    if not is_rag_ready:
        return []
...
async def reload_instructions():
    """
    Full reindex after adding/changing PDFs/TXTs.
    """
    try:
