...
def load_instructions():
    ...
    for file_path in all_files:
        ...
        # Jeśli nie znaleziono, dodajemy pustą listę
        device_tag = ",".join(matched_devices) if matched_devices else "unknown"

        for chunk in chunks:
            all_chunks.append(chunk)
            # W metadanych przechowujemy źródło i tag urządzenia
...
def search_instructions(query: str, top_k: int = 10) -> List[Tuple[str, Dict]]:
    ...
    if target_device:
        ...
        if filtered:
            ...
            return filtered[:top_k]  # zwracamy tylko tyle, ile żądano
        else:
            print(f"⚠️ Nie znaleziono fragmentów dla urządzenia '{target_device}', używam ogólnych")
            # Jeśli nie ma pasujących, zwracamy ogólne (ale ostrzegamy)
            return fragments_with_meta[:top_k]
    else:
        # Brak wskazania urządzenia – zwracamy ogólne wyniki
        return fragments_with_meta[:top_k]
...
async def chat(request: ChatRequest):
    ...
    # System prompt – teraz z wyraźnym nakazem używania kontekstu
    system_prompt = f"""
            ...
            for attempt in range(max_retries):
                try:
                    response = await client.post(
                        ...
                        json={
                            "model": "llama-3.1-8b-instant",  # lżejszy model
                            "messages": history,
                            "temperature": 0.3,
                            "max_tokens": 500,
                            "top_p": 0.9
