...
async def chat(request: ChatRequest):
    ...
    # --- ULEPSZONY SYSTEM PROMPT (pełne opisy, podział na akapity) ---
    system_prompt = f"""
...
**FORMAT ODPOWIEDZI**:
1. Odpowiedź musi zawierać **pełny cykl czynności od początku do końca**, opisany w postaci numerowanej listy krok po kroku.
2. Każdy krok pisz w osobnej linii, poprzedzony emoji: 1️⃣, 2️⃣, 3️⃣ itd.
3. **Między krokami zostaw pustą linię (akapit)**, aby odpowiedź była czytelna.
4. Opisuj **dokładnie, co użytkownik ma nacisnąć, jakie kody wprowadzić, jak wejść w tryb programowania** – nie pomijaj żadnych czynności.
5. Maksymalnie 10 kroków.
6. Nie dodawaj wstępów, podsumowań ani wyjaśnień technicznych – tylko lista kroków.
...
2️⃣ Usuń starego użytkownika: naciśnij 8. Dioda Mode zaświeci się na czerwono, dioda Door na pomarańczowo. Wprowadź trzycyfrowy numer użytkownika (np. 001). Dioda Mode zacznie migać na czerwono – teraz wprowadź Kod Programowania, aby potwierdzić usunięcie. Usłyszysz trzy krótkie sygnały.
...
**ZASADY SPECJALNE**:
- Jeśli krok dotyczy zasilania, napisz po prostu: "Sprawdź zasilanie".
- Jeśli problemu nie da się rozwiązać na podstawie kontekstu, napisz dokładnie:
   "Nie znalazłem tej informacji w instrukcjach. Proszę skontaktować się z serwisem."
- Nie używaj sformułowań typu "na podstawie instrukcji", "zgodnie z dokumentacją" – pisz po prostu kroki.

**DLA URZĄDZEŃ (jeśli w kontekście występują)**:
- Rosslare / AC-B31 / AC-B32: jeśli w kontekście są opisane procedury (tryb programowania, usuwanie użytkownika, dodawanie użytkownika, zmiana kodu) – przepisz je dokładnie jako kroki 1️⃣, 2️⃣, 3️⃣... i upewnij się, że opisano sposób wejścia w tryb programowania oraz użycie Kod Programowania.
- Paradox, Satel, Bosch, Siemens, 3xLogic, Provision, Hikvision: korzystaj tylko z tego, co faktycznie jest w kontekście. Jeśli kontekst zawiera sekwencje przycisków i kodów – podaj je w całości, od pierwszego do ostatniego kroku. Jeśli nie ma opisu wejścia w tryb, a jest on potrzebny – sprawdź, czy kontekst go zawiera; jeśli nie, nie wymyślaj, tylko poproś o kontakt z serwisem.
...
"""
    ...
    # Если контекст пуст, а устройство определено – быстрый ответ (экономия токенов)
    ...
    # Управление историей (системный + последние 5 сообщений)
    if not history:
                ...
                try:
                    resp = await client.post(
                        ...
                        json={
                            ...
                            "messages": history,
                            "temperature": 0.1,
                            "max_tokens": 800,   # zwiększone, aby odpowiedzi mogły być dłuższe
                            "top_p": 0.9
                        },
                        timeout=30.0
                    )
