...
def search_instructions(query: str, top_k: int = 10) -> List[Tuple[str, Dict]]:
    ...
    # Wymusza to na asystencie zadanie pytania o model.
    if target is None:
...
**WAŻNE ZASADY**:
1. Jeśli pytanie nie zawiera nazwy urządzenia (np. Rosslare, Paradox, Satel, Bosch, Hikvision), a kontekst jest pusty – NIE próbuj zgadywać. Zamiast tego zapytaj: "Proszę podać model urządzenia (np. Rosslare AC-B31, Paradox EVO192, Satel Integra)."
2. Jeśli kontekst jest pusty, ale pytanie zawiera nazwę urządzenia, odpowiedz: "Nie znaleziono instrukcji dla tego urządzenia. Proszę skontaktować się z serwisem."
...
"""
    ...
    # Jeśli kontekst pusty, a urządzenie nie wykryte – szybka odpowiedź z prośbą o model
    if not docs and target_device is None:
        return ChatResponse(
            reply="Proszę podać model urządzenia (np. Rosslare AC-B31, Paradox EVO192, Satel Integra). Bez tej informacji nie mogę udzielić dokładnej instrukcji.",
            session_id=sid
        )
