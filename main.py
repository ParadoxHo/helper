...
async def chat(req: ChatRequest):
    ...
    # ДОБАВЛЯЕМ ПРОВЕРКУ: если устройство есть, но контекст пуст
    if device is not None and not context:
        return ChatResponse(
            reply=f"Nie znaleziono instrukcji dla urządzenia {device}. Proszę skontaktować się z serwisem.",
            session_id=sid,
            source="rag"
        )
...
async def list_devices():
    """Zwraca listę urządzeń, dla których są instrukcje"""
    if not rag_ready or collection is None:
        return {"devices": []}
    # Pobieramy metadane (ograniczone do 1000 rekordów)
