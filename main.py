...
async def reload_instructions():
    ...
    try:
        # Опционально: очистим коллекцию перед загрузкой (можно раскомментировать)
        # if collection:
...
# При старте приложения загружаем инструкции и модель
...
async def startup_event():
    ...
    load_instructions()
    # Прогреваем модель эмбеддингов (загружаем в фоне, не блокируя старт)
    import asyncio
    asyncio.create_task(background_load_model())

async def background_load_model():
    """Фоновая загрузка модели эмбеддингов, чтобы не задерживать старт"""
    try:
        get_embedding_model()
    except Exception as e:
        print(f"⚠️ Nie udało się załadować modelu w tle: {e}")
