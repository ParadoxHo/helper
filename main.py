...
def split_text_into_chunks(text: str, chunk_size: int = 300, overlap: int = 30) -> List[str]:
    """Dzieli tekst na mniejsze fragmenty (300 słów) dla lepszego dopasowania."""
    words = text.split()
    chunks = []
...
def search_instructions(query: str, top_k: int = 7) -> List[str]:
    """Zwraca listę najbardziej pasujących fragmentów (zwiększone top_k)."""
    global collection
    init_chroma()
...
async def chat(request: ChatRequest):
    ...
    # Ulepszony system prompt – wymuszamy odpowiedź tylko z kontekstu
    system_prompt = f"""
...
**ODPOWIADAJ WYŁĄCZNIE NA PODSTAWIE TEGO KONTEKSTU.**
Jeśli kontekst nie zawiera odpowiedzi na pytanie, powiedz: "Przepraszam, nie znalazłem tej informacji w instrukcjach. Proszę skontaktować się z serwisem."
...
2. Używaj wyłącznie informacji z kontekstu. Nie dodawaj własnej wiedzy.
3. Jeśli kontekst zawiera instrukcję – podaj ją krok po kroku.
4. Jeśli nie ma kontekstu lub nie zawiera odpowiedzi, poinformuj o tym i zaproponuj kontakt z serwisem.
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={
                    "model": "mixtral-8x7b-32768",  # Zmieniono na model z wyższym limitem
                    "messages": history,
                    "temperature": 0.3,
                    "max_tokens": 600,  # Zmniejszono, by oszczędzać tokeny
                    "top_p": 0.9
                },
                timeout=30.0
            )
            ...
            if response.status_code != 200:
                ...
                # Jeśli 429, możemy zwrócić przyjazny komunikat
                if response.status_code == 429:
                    return ChatResponse(
                        reply="Przepraszam, osiągnięto limit zapytań. Proszę spróbować za chwilę.",
                        session_id=sid
                    )
...
# Jawne obsłużenie OPTIONS dla preflight
@app.options("/chat")
...
async def reload_instructions():
    try:
        # Opcjonalnie można wyczyścić kolekcję przed ponownym załadowaniem
        # if collection:
