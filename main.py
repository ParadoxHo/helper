...
# Разрешённые домены для CORS (включаем тот, с которого идет запрос)
ALLOWED_ORIGINS = [
...
app = FastAPI(
    title="Wsparcie Techniczne AZS z RAG (corrected CORS)",
    lifespan=lifespan
)
...
def search_instructions(query: str, top_k: int = 15) -> List[Tuple[str, Dict]]:
    ...
    for i in range(len(results["documents"][0])):
        frags.append(
            (results["documents"][0][i], results["metadatas"][0][i] if results.get("metadatas") else {})
        )
...
async def options_chat(request: Request):
    # ВАЖНО: браузер делает preflight OPTIONS /chat
    # Если тут нет Access-Control-Allow-Origin, запрос будет заблокирован.
    origin = (request.headers.get("origin") or "").rstrip("/")

    if origin and origin in ALLOWED_ORIGINS:
        # Браузер может прислать список запрошенных заголовков
...
6. Odpowiadaj TYKO po polsku, w formie numerowanej listy z emoji (1️⃣, 2️⃣, 3️⃣...). Między krokami zostaw pustą linię (akapit).
...
1️⃣ Wejdź w tryb programowania: naciśnij i przytrzymaj przycisk # przez 2 sekundy. Dioda Mode zgaśnie, dioda Door zapali się na czerwono. Wprowadź Kod Programowania (fabrycznie 1234). Po poprawnym wprowadzeniu dioda Door zapali się na zielono.

2️⃣ Usuń starego użytkownika: naciśnij 8. Dioda Mode zapali się na czerwono, dioda Door na pomarańczowo. Wprowadź trzycyfrowy numer użytkownika (np. 001). Dioda Mode zacznie migać na czerwono – wprowadź Kod Programowania, aby potwierdzić usunięcie.

3️⃣ Dodaj nowego użytkownika: naciśnij 7. Dioda Door zapali się na pomarańczowo. Wprowadź ten sam numer użytkownika (001). Dioda Mode zacznie migać na zielono – wprowadź nowy czterocyfrowy kod.

4️⃣ Wyjdź z trybu programowania: naciśnij i przytrzymaj # przez 2 sekundy. Usłyszysz trzy sygnały, dioda Door zgaśnie, a dioda Mode zapali się na zielono.
"""

    # Если нет конtekста и нет устройства
    ...
    # Если нет контекста, но устройство известно
