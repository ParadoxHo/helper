...
# Отключаем телеметрию ChromaDB (чтобы убрать предупреждения)
...
# --- Настройка CORS (только через middleware, без ручных OPTIONS) ---
ALLOWED_ORIGINS = [
    "https://assistics.netlify.app",
    "https://pagggge.vercel.app",
    "https://assistics.vercel.app",      # новый домен
    "http://localhost:8000",
    "http://127.0.0.1:8000"
]
...
# --- Ключ API Groq (обязательно должен быть в переменных окружения) ---
...
def load_instructions():
    ...
    for file_path in all_files:
        ...
        # Определяем, какие устройства соответствуют имени файла
        matched_devices = []
...
def extract_device_from_query(query: str) -> Optional[str]:
    """Извлекает название устройства из запроса пользователя."""
...
def search_instructions(query: str, top_k: int = 7) -> List[Tuple[str, Dict]]:
    """Ищет релевантные фрагменты и возвращает их вместе с метаданными."""
    global collection
    init_chroma()
    ...
    # Сначала ищем общие результаты
    results = collection.query(
        query_embeddings=[query_embedding],
        n_results=top_k * 2  # берём больше, потом отфильтруем
    )

    if not results or not results['documents']:
        return []

    # Собираем фрагменты с метаданными
    fragments_with_meta = []
    ...
    # Если в запросе найдено устройство, фильтруем по нему
...
"""
    ...
    # Вызов Groq с повторными попытками
    try:
            ...
            for attempt in range(max_retries):
                try:
                    response = await client.post(
                        ...
                        json={
                            "model": "llama-3.1-8b-instant",  # стабильная, быстрая модель
                            "messages": history,
                            "temperature": 0.3,
                            "max_tokens": 500,
                            "top_p": 0.9
...
async def list_models():
    """Эндпоинт для проверки доступных моделей Groq."""
    if not GROQ_API_KEY:
