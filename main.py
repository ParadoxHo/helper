...
<body>
    ...
    <script>
        // Umieść cały kod w funkcji, która uruchomi się po załadowaniu DOM
        document.addEventListener('DOMContentLoaded', function() {
            console.log('DOM załadowany, inicjalizacja...');
            
            // Pobierz elementy z zabezpieczeniem
            ...
            const elements = { messagesDiv, input, sendButton, statusBadge };
            for (const [name, el] of Object.entries(elements)) {
                if (!el) {
                    console.error(`Element ${name} nie istnieje!`);
                    return;
                }
            }
            
            console.log('✅ Wszystkie elementy znalezione');
                    ...
                    for (const model of models) {
                        try {
                            ...
                            response = await window.puter.ai.chat(text, {
                                model: model,
                                system_prompt: `Jesteś inżynierem wsparcia technicznego dla systemów bezpieczeństwa na stacjach benzynowych w Polsce. Specjalizacja: kamery, alarmy, kontrola dostępu. Odpowiadaj TYLKO po polsku, krótko i konkretnie. Zawsze zaczynaj od diagnostyki: sprawdź zasilanie, kable, połączenia.`
                            });
            ...
            // Event listeners - TERAZ WSZYSTKO DZIAŁA!
